...
/**
 * Copyright 2011 Clay Tablet Technologies Inc.
 *  
 * 
 * <p>
 */
public class FastBase64 {
	...
	public final static String From64(String s)	
	{
		if (s == null)
			return null;
		...
		byte[] decoded = decodeFast(s);
		String s64 = null;
		try
		{
			s64 = new String(decoded, "UTF-8"); 
		}
		catch (Exception e) 
		{
			s64 = new String(decoded);
		}
		return s64;		
	}
	
	public final static String To64(String s)
	{
		if (s == null)
			s = "";
		...
		byte[] stringArray;
        try {
            stringArray = s.getBytes("UTF-8");  // use appropriate encoding string!
        } catch (Exception ignored) {
            stringArray = s.getBytes();  // use locale default rather than croak
        }
        
        return encodeToString(stringArray); 
	}
	
	public final static String decodeToString(String str)
	{
		byte[] decodedBytes = decodeFast(str);
